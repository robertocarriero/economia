...
def circulacion_monetaria(request):
    circul_por_mes = None
    grafico = None
    ...
    if request.method == 'POST':
        ...
        circul_filtrados = [{'d': item['d'], 'v': item['v']} for item in data if (not fecha_inicio or item['d'] >= fecha_inicio) and (not fecha_fin or item['d'] <= fecha_fin)]

        # Agrupar los datos por mes
        circul_por_mes = {}
        for item in circul_filtrados:
            fecha = datetime.strptime(item['d'], '%Y-%m-%d')
            fecha_formateada = fecha.strftime('%d-%m-%Y')
            mes = fecha.strftime('%B %Y')
            if mes not in circul_por_mes:
                circul_por_mes[mes] = []
            valor_formateado = formatear_valor(item['v'])
            circul_por_mes[mes].append({'d': fecha_formateada, 'v': valor_formateado})
        ...
        plt.ylabel('Circulación en Millones de Pesos')
        plt.title('Circulación Mensuales')
    ...
    return render(request, 'circulacion/circulacion.html', {'circul_por_mes': circul_por_mes, 'grafico': grafico})
