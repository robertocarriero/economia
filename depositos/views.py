...
def depositos(request):
    ...
    if request.method == 'POST':
        ...
        url = "https://api.estadisticasbcra.com/depositos"
        headers = {
        ...
        dep_filtrados = [{'d': item['d'], 'v': item['v']} for item in data if (not fecha_inicio or item['d'] >= fecha_inicio) and (not fecha_fin or item['d'] <= fecha_fin)]

        # Agrupar los datos por mes
        dep_por_mes = {}
        for item in dep_filtrados:
            fecha = datetime.strptime(item['d'], '%Y-%m-%d')
            fecha_formateada = fecha.strftime('%d-%m-%Y')
            mes = fecha.strftime('%B %Y')
            if mes not in dep_por_mes:
                dep_por_mes[mes] = []
            valor_formateado = formatear_valor(item['v'])
            dep_por_mes[mes].append({'d': fecha_formateada, 'v': valor_formateado})

        # Crear el gráfico de línea
        plt.figure(figsize=(6, 6))
        for mes, depositos in dep_por_mes.items():
        ...
        plt.ylabel('Depósitos en Millones de Pesos')
        plt.title('Depósitos Mensuales')
    ...
    return render(request, 'depositos/deposito.html', {'dep_por_mes': dep_por_mes, 'grafico': grafico})
