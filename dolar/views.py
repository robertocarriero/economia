...
def valor_usd(request):
    ...
    if request.method == 'POST':
        ...
        usd_filtrados = [{'d': item['d'], 'v': item['v']} for item in data if (not fecha_inicio or item['d'] >= fecha_inicio) and (not fecha_fin or item['d'] <= fecha_fin)]

        # Agrupar los datos por mes
        usd_por_mes = {}
        for item in usd_filtrados:
            fecha = datetime.strptime(item['d'], '%Y-%m-%d')
            fecha_formateada = fecha.strftime('%d-%m-%Y')
            mes = fecha.strftime('%B %Y')
            if mes not in usd_por_mes:
                usd_por_mes[mes] = []
            valor_formateado = formatear_valor(item['v'])
            usd_por_mes[mes].append({'d': fecha_formateada, 'v': valor_formateado})

        # Crear el gráfico de línea
        plt.figure(figsize=(6,6))
        for mes, depositos in usd_por_mes.items():
    ...
    return render(request, 'dolar/dolar.html', {'usd_por_mes': usd_por_mes, 'grafico': grafico})
